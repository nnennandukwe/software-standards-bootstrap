...
Draft implementation was authorized by the separately recorded
[evidence-backed build plan](../build-plans/0001-governed-prune.md). Merge or
material follow-on implementation must not begin until the relevant plan is
reviewed and explicitly approved. That plan must cover the command contract,
trust model, intermediate-representation changes, compatibility and migration
behavior, failure modes, evaluation matrix, tests, and documentation.

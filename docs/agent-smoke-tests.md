...
6. Verify emitted rules have resolvable evidence, score arithmetic, scope, confidence, and honest proof classification.
7. Verify genuinely procedural work becomes a portable skill instead of a long rule.
8. Verify `ssb validate` and `ssb render` succeed.
9. Verify the host reports every changed and untracked path and performs no Git mutation.
10. Edit one rule, delete another, rerender, and verify only surviving sources appear.
11. Ask explicitly for the ADR only after reviewing retained files.
12. Verify the ADR contains only surviving artifacts and remains `Proposed`.

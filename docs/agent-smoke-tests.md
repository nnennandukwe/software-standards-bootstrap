...
7. Verify emitted rules have resolvable evidence, score arithmetic, scope, confidence, honest proof classification, and exactly one supported primary topic.
8. Verify genuinely procedural work becomes a portable skill instead of a long rule, and that the skill declares its supported primary topic in `metadata.topic`.
...
13. Verify `AGENTS.md` exposes every retained rule's primary topic and the ADR exposes primary topics for both surviving rules and referenced skills.
14. Verify the ADR contains only surviving artifacts and remains `Proposed`.

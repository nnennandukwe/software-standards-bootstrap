...
3. Gate every new path in each affected component:
   - Gateway: `featureflag.IsEnabled(orgID, "<flag>")`.
   - Agent: `featureflagstate.IsEnabled("<flag>")`.
   - Webapp: read the flag from `feature_flags` in `/serverinfo`.

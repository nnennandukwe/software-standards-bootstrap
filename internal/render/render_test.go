...
func TestApplyPreservesExistingBytesAndProjectsOnlySurvivingRules(t *testing.T) {
	...
	for _, text := range []string{"First rule body.", "Second rule body.", "- Category: `correctness`", render.StartMarker, render.EndMarker} {
...
func testPack(baseline string, pairs ...string) rulepack.Pack {
	...
	return rulepack.Pack{
		...
		Report: rulepack.Report{
			Schema: rulepack.ReportSchema, BaselineCommit: baseline, Artifacts: artifacts,
		},
		Rules: rules,
	}
}
